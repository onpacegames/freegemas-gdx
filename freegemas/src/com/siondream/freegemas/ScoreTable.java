...
public class ScoreTable implements Input.TextInputListener {
	...
	// TODO this is a bit messy by trying to shoehorn try-with-resources, it could be better.
	public void saveScore() {
		try (StringWriter writer = new StringWriter()) {
			try (XmlWriter xml = new XmlWriter(writer)) {
				try (XmlWriter scoresElement = xml.element("scores")) {
					XmlWriter scoreElement = scoresElement;
					try {
						int numScores = _scores.size;
						for (int i = 0; i < numScores; ++i) {
							try (XmlWriter tempScoreElement = scoreElement.element("score").attribute("name", _scores.get(i).getFirst()).attribute("points", _scores.get(i).getSecond()).pop()) {
